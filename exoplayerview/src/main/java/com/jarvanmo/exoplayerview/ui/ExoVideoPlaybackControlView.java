...
//    <!--<enum getDisplayName="bottom_landscape" value="0b0001"/>-->
    ...
    private final View enterFullscreen;//竖屏的全屏按钮
    private final View exitFullscreen;//横屏的全屏按钮
    ...
    private boolean enableGesture = false;//是否可以滑动
...
//        loadingBar = findViewById(R.id.exo_player_loading);
    ...
    private CharSequence generateFastForwardOrRewindTxt(long changingTime) {
        ...
        TypedArray a = getContext().obtainStyledAttributes(typedValue.data, null);
    ...
    private static final String TAG = "ExoVideoPlaybackControl";
    ...
    private void updatePlayPauseButton() {
        ...
        if (pauseButton != null) {
            ...
            pauseButton.setVisibility(!playing && ivLoading.getVisibility() == GONE ? View.GONE : View.VISIBLE);
        }
    ...
    public void setVisibilityCallback(MultiQualitySelectorAdapter.VisibilityCallback
                                              qualityVisibilityCallback) {
...
//            loadingBar.setVisibility(GONE);
    ...
    private final class ComponentListener implements
        ...
        public void onScrubStop(TimeBar timeBar, long position, boolean canceled) {
            scrubbing = false;
            if (!canceled && player != null && enableGesture) {
...
//            Log.e(TAG, "onPlayerStateChanged: " + playWhenReady);
            if (playbackState != Player.STATE_IDLE && centerError != null && centerError.getVisibility() == VISIBLE) {
                centerError.setVisibility(GONE);
            }

            if (playWhenReady && (playbackState == Player.STATE_IDLE || playbackState == Player.STATE_BUFFERING)) {
                removeCallbacks(hideAction);
                showUtilHideCalled();
                showLoading(true);
