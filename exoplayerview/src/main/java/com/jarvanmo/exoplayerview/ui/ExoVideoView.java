...
public class ExoVideoView extends FrameLayout implements ExoVideoPlaybackControlView.VideoViewAccessor {
    ...
    public void startVideo(boolean isPlay, OnClickListener listener) {
        Assertions.checkState(controller != null);
        if (!isPlay) Assertions.checkArgument(listener != null);
        if (mPlayWhenReady) return;
        controller.setOnPlayClickListener(view -> {
            if (isPlay) controller.startVideo();
            else {
...
//    }
    ...
    private boolean isPlayingAd() {
        return player != null && player.isPlayingAd() && player.getPlayWhenReady() && player.getPlaybackState() != Player.STATE_ENDED;
    }
...
//        hideArtwork();
    ...
    private void hideArtwork() {
        if (artworkView != null) {
            artworkView.animate().alpha(1.0f).alpha(0f).setDuration(100).start();
    ...
    private void createExoPlayer(MediaSourceCreator creator) {
        ...
        internalPlayer.addAnalyticsListener(creator.getEventLogger());
        internalPlayer.addMetadataOutput(creator.getExoEventLogger());
