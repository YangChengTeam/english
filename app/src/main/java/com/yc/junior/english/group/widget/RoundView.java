package com.yc.junior.english.group.widget;
...
/**
 * Created by wanglin  on 2017/7/24 14:40.
 */
...
public class RoundView extends AppCompatImageView {
    ...
    public RoundView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.roundView, defStyle, 0);
        try {
            ...
            // 获取边界的宽度
            mBorderWidth = ta.getDimensionPixelSize(R.styleable.roundView_border_width, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 30, getResources().getDisplayMetrics()));
            // 获取边界的颜色
            mBorderColor = ta.getColor(R.styleable.roundView_border_color, DEFAULT_BORDER_COLOR);
            mBorderOverlay = ta.getBoolean(R.styleable.roundView_border_overlay, DEFAULT_BORDER_OVERLAY);
        } finally {
            ta.recycle();
        }
